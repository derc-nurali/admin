...
export const Achievements: ComponentType = () => {
  ...
  const {
    ...
    queryKeyword,
    columns,
  }: any = useCatalog(ACHIEVEMENTS_RESOURCES);

  const navigation: TabNavItemsProps[] = [
    {
      label: t('all', 'Все'),
      path: dynamicRoute(ROUTES.ACHIEVEMENTS.ROOT),
      counter: total,
    },
    {
      label: t('drafts', 'Черновики'),
      path: dynamicRoute(ROUTES.ACHIEVEMENTS.DRAFTS),
      counter: drafts,
    },
  ];
  ...
  return (
    <Container>
      <BackButton />
      <PageTitle title={t('achievements', 'Достижения')} />
      <Grid container mb={2} columnSpacing={2}>
        ...
        >
          <DataTableCreateButton entity={ENTITY_ACHIEVEMENT} />
        </Grid>
      </Grid>
      <DataTableBrim
        actions={[
          <DataTableCreateButton entity={ENTITY_ACHIEVEMENT} variant="small" />,
        ]}
      >
