...
export const Services: ComponentType = () => {
  ...
  const {
    ...
    queryCreatedAtFrom,
    columns,
  } = useCatalog(SERVICES_RESOURCES);

  const navigation: TabNavItemsProps[] = [
    {
      label: t('all', 'Все'),
      path: dynamicRoute(ROUTES.SERVICES.ROOT),
      counter: total,
    },
    {
      label: t('drafts', 'Черновики'),
      path: dynamicRoute(ROUTES.SERVICES.DRAFTS),
      counter: drafts,
    },
  ];
  ...
  return (
    <Container>
      <BackButton />
      <PageTitle title={t('services', 'Услуги')} />
      <Grid container mb={2} columnSpacing={2}>
        ...
        >
          <DataTableCreateButton entity={ENTITY_SERVICE} />
        </Grid>
      </Grid>
      <DataTableBrim
        actions={[
          <DataTableCreateButton entity={ENTITY_SERVICE} variant="small" />,
          <PeriodsSelectForm
