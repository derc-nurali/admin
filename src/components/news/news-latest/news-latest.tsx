...
import { getLatest } from '../../../thunks/news-thunks';
...
export const NewsLatest: ComponentType = () => {
  const dispatch = useDispatch();
  const { flow } = useWorkflow();
  const { currentLocale } = useLanguage();
  const { latest } = useSelector((state: RootStateOrAny) => state[ENTITY_NEWS]);
  ...
  const items = map(hits, ({ id, title }, idx) => (
    <ListItem disablePadding key={idx}>
      <ListItemButton to={newsEditRoute({ id })} component={Link}>
        <ListItemIcon>
          <SvgIcon
