...
export const Documents: ComponentType = () => {
  ...
  const {
    ...
    queryKeyword,
    columns,
  }: any = useCatalog(DOCUMENTS_RESOURCES);

  const navigation: TabNavItemsProps[] = [
    {
      label: t('all', 'Все'),
      path: dynamicRoute(ROUTES.DOCUMENTS.ROOT),
      counter: total,
    },
    {
      label: t('drafts', 'Черновики'),
      path: dynamicRoute(ROUTES.DOCUMENTS.DRAFTS),
      counter: drafts,
    },
  ];
  ...
  return (
    <Container>
      <BackButton />
      <PageTitle title={t('documents', 'Документы')} />
      <Grid container mb={2} columnSpacing={2}>
        ...
        >
          <DataTableCreateButton entity={ENTITY_DOCUMENT} />
        </Grid>
      </Grid>
      <DataTableBrim
        actions={[
          <DataTableCreateButton entity={ENTITY_DOCUMENT} variant="small" />,
        ]}
      >
