...
import { ORGANIZATIONS_RESOURCES } from './organizations-resource';
import { ROUTES } from '../../constants/routes/route-constants';

export const Organizations: ComponentType = () => {
  ...
  const {
    ...
    queryKeyword,
    columns,
  }: any = useCatalog(ORGANIZATIONS_RESOURCES);

  const navigation: TabNavItemsProps[] = [
    {
      label: t('all', 'Все'),
      path: dynamicRoute(ROUTES.ORGANIZATIONS.ROOT),
      counter: total,
    },
  ];
  ...
  return (
    <Container>
      <BackButton />
      <PageTitle title={t('organizations', 'Организации')} />
      <Grid container mb={2} columnSpacing={2}>
        ...
        >
          <DataTableCreateButton entity={ENTITY_ORGANIZATION} />
        </Grid>
      </Grid>
      <DataTableBrim
        actions={[
