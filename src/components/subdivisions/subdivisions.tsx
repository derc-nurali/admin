...
export const Subdivisions: ComponentType = () => {
  ...
  const {
    ...
    queryKeyword,
    columns,
  }: any = useCatalog(SUBDIVISIONS_RESOURCES);

  const navigation: TabNavItemsProps[] = [
    {
      label: t('all', 'Все'),
      path: dynamicRoute(ROUTES.SUBDIVISIONS.ROOT),
      counter: total,
    },
    {
      label: t('drafts', 'Черновики'),
      path: dynamicRoute(ROUTES.SUBDIVISIONS.DRAFTS),
      counter: drafts,
    },
  ];
  ...
  return (
    <Container>
      <BackButton />
      <PageTitle title={t('subdivisions', 'Подразделения')} />
      <Grid container mb={2} columnSpacing={2}>
        ...
        >
          <DataTableCreateButton entity={ENTITY_SUBDIVISION} />
        </Grid>
      </Grid>
      <DataTableBrim
        actions={[
          <DataTableCreateButton entity={ENTITY_SUBDIVISION} variant="small" />,
        ]}
      >
