...
export const Activities: ComponentType = () => {
  ...
  const {
    ...
    queryKeyword,
    columns,
  }: any = useCatalog(ACTIVITIES_RESOURCES);

  const navigation: TabNavItemsProps[] = [
    {
      label: t('all', 'Все'),
      path: dynamicRoute(ROUTES.ACTIVITIES.ROOT),
      counter: total,
    },
    {
      label: t('drafts', 'Черновики'),
      path: dynamicRoute(ROUTES.ACTIVITIES.DRAFTS),
      counter: drafts,
    },
  ];
  ...
  return (
    <Container>
      <BackButton />
      <PageTitle title={t('activity', 'Деятельность')} />
      <Grid container mb={2} columnSpacing={2}>
        ...
        >
          <DataTableCreateButton entity={ENTITY_ACTIVITY} />
        </Grid>
      </Grid>
      <DataTableBrim
        actions={[
          <DataTableCreateButton entity={ENTITY_ACTIVITY} variant="small" />,
        ]}
      >
