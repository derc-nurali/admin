...
export const Faq: ComponentType = () => {
  ...
  const {
    ...
    queryCreatedAtFrom,
    columns,
  } = useCatalog(FAQ_RESOURCES);

  const navigation: TabNavItemsProps[] = [
    {
      label: t('all', 'Все'),
      path: dynamicRoute(ROUTES.FAQ.ROOT),
      counter: total,
    },
    {
      label: t('drafts', 'Черновики'),
      path: dynamicRoute(ROUTES.FAQ.DRAFTS),
      counter: drafts,
    },
  ];
  ...
  return (
    <Container>
      ...
      <Grid container mb={2} columnSpacing={2}>
        ...
        >
          <DataTableCreateButton entity={ENTITY_FAQ} />
        </Grid>
      </Grid>
      <DataTableBrim
        actions={[
          <DataTableCreateButton entity={ENTITY_FAQ} variant="small" />,
          <PeriodsSelectForm
